  ...
  <!-- ################################################################################################ -->
      ...
      <?php
        ...
        if ($result=mysqli_query($link, $SQL)){
            ...
            echo "<td class='tdBlack'>名稱</td>";
            echo "<td class='tdBlack'>生日</td>";
            echo "<td class='tdBlack'>電話</td>";
            echo "<td class='tdBlack'>電子郵件信箱</td>";
            echo "<td class='tdBlack'>身份</td>";
            echo "<td class='tdBlack'>操作</td>";
            $i = 1;
            while($row=mysqli_fetch_assoc($result)){
                echo "<tr>";
                echo "<td class='tdBlack'>".($i++)."</td>";
                echo "<td class='tdBlack'>".$row["uName"]."</td>";
                echo "<td class='tdBlack'>".$row["birthday"]."</td>"; 
                echo "<td class='tdBlack'>".$row["tel"]."</td>"; 
                echo "<td class='tdBlack'>".$row["email"]."</td>"; 
                echo "<td class='tdBlack'>".$row["uRole"]."</td>"; 
                echo "<td class='tdBlack'><a href='deleteUser.php?uNo=".$row["uNo"]."'' style='color: #345998;width:15%;'><u>刪除</u></a></td>";
                echo "</tr>";
            }
            echo "<form action='addUser.php' method = 'post' style = 'width: 100%;' enctype = 'multipart/form-data'>";
            echo "<tr>
                <td></td>
                <td class='tdBlack'><input type='text' class='input1' style='width: 150px;' name='uName' required></td>
                <td class='tdBlack'><input type='text' class='input1' style='width: 170px;' name='birthday' onfocus=(this.type='date') id='date'></td>
                <td class='tdBlack'><input type='text' class='input1' style='width: 150px;' name='tel' required></td>
                <td class='tdBlack'><input type='text' class='input1' style='width: 220px;' name='email' required></td>
                <td class='tdBlack'><input type='text' class='input1' style='width: 80px;' name='uRole' required></td>
                <td class='tdBlack'><button type='submit' style='width:70px;color: #345998; border:0;background-color: rgba(0, 0, 0, 0);cursor:pointer;'><u>新增</u></button></td></tr>";
            echo "</table>";
            echo "</form>";
        }
    ?>
