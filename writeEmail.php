...
<div  style="background:linear-gradient(180deg, #FFFFFF 0%, #FFFFFF 135px,#bbf0aa 135px,  #bbf0aa 100%);"> 
  ...
  <!-- ################################################################################################ -->
      ...
      <?php
        ...
        $SQL="SELECT sEmail FROM subscriber";
        ...
        if(isset($_POST['chkall'])){
            ...
            if(isset($_POST['email'])){
                ...
                echo "alert('請至少選擇一位訂閱者！');";
                echo "</script>";
                echo "<meta http-equiv='Refresh' content='0; url=subscribeManagement.php'>";
            }  
        }
        $_SESSION['email'] = $email;
        echo "<h1 style='color:#000; font-size:27px;'>撰寫郵件</h1>";
    ?>
    <form action='sendToSubscriber.php' method='post'>
        <textarea class='textarea1' name='title' style='width:500px;height:50px;' placeholder='信件主旨' required></textarea><br/>
        <textarea class='textarea1' name='content' placeholder='請在此輸入信件內容'></textarea><br/>
        <button type='submit' class='button1'>送出</button>
    </form>    
    </div>
    </div>
